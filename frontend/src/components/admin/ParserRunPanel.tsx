...
export default function ParserRunPanel({ target, token, onCompleted }: { target: ParserTarget; token: string | null; onCompleted?: () => void | Promise<void> }) {
  ...
  return (
    <form onSubmit={submit} className="grid gap-3 rounded-2xl border border-sky-100 bg-sky-50 p-4 lg:grid-cols-[1.2fr_repeat(4,minmax(0,1fr))_auto]">
      ...
      <label className="text-xs font-bold text-slate-600">Широта<input required type="text" inputMode="decimal" value={lat} onChange={(event) => setLat(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" /></label>
      <label className="text-xs font-bold text-slate-600">Долгота<input required type="text" inputMode="decimal" value={lon} onChange={(event) => setLon(event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm" /></label>
