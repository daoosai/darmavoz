...
export default function WaterSepticModerationPanel({ token }: { token: string | null }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      ...
      {editTarget ? <div className="fixed inset-0 z-[70] flex items-end justify-center bg-slate-900/40 p-4 sm:items-center" role="dialog" aria-modal="true"><form onSubmit={submitEdit} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl"><div className="flex items-start justify-between gap-4"><h3 className="text-lg font-black text-slate-900">{editTarget.kind === "water" ? "Редактирование точки воды" : "Редактирование септика"}</h3><button type="button" onClick={() => setEditTarget(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Закрыть"><X className="h-5 w-5" /></button></div>{editTarget.kind === "water" ? <div className="mt-4 space-y-3"><label className="block text-sm font-bold">Тип воды<select value={waterEditForm.water_type} onChange={(event) => setWaterEditForm((current) => ({ ...current, water_type: event.target.value as "free" | "paid" }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal"><option value="free">Бесплатная</option><option value="paid">Платная</option></select></label><label className="block text-sm font-bold">Название<input value={waterEditForm.name} onChange={(event) => setWaterEditForm((current) => ({ ...current, name: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="block text-sm font-bold">Источник<input required value={waterEditForm.source} onChange={(event) => setWaterEditForm((current) => ({ ...current, source: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="block text-sm font-bold">Адрес<input required value={waterEditForm.address} onChange={(event) => setWaterEditForm((current) => ({ ...current, address: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Широта<input required type="number" step="any" value={waterEditForm.lat} onChange={(event) => setWaterEditForm((current) => ({ ...current, lat: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="text-sm font-bold">Долгота<input required type="number" step="any" value={waterEditForm.lon} onChange={(event) => setWaterEditForm((current) => ({ ...current, lon: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div><label className="block text-sm font-bold">Телефон<input value={waterEditForm.phone} onChange={(event) => setWaterEditForm((current) => ({ ...current, phone: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label>{waterEditForm.water_type === "paid" ? <div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Цена<input required type="number" min="0.01" step="0.01" value={waterEditForm.price} onChange={(event) => setWaterEditForm((current) => ({ ...current, price: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="text-sm font-bold">Единица<input required value={waterEditForm.price_unit} onChange={(event) => setWaterEditForm((current) => ({ ...current, price_unit: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div> : null}<label className="block text-sm font-bold">Описание<textarea value={waterEditForm.description} onChange={(event) => setWaterEditForm((current) => ({ ...current, description: event.target.value }))} className="mt-1 min-h-20 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div> : <div className="mt-4 space-y-3"><label className="block text-sm font-bold">Телефон<input required value={septicEditForm.phone} onChange={(event) => setSepticEditForm((current) => ({ ...current, phone: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="block text-sm font-bold">Адрес<input required value={septicEditForm.address} onChange={(event) => setSepticEditForm((current) => ({ ...current, address: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Широта<input required type="number" step="any" value={septicEditForm.lat} onChange={(event) => setSepticEditForm((current) => ({ ...current, lat: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="text-sm font-bold">Долгота<input required type="number" step="any" value={septicEditForm.lon} onChange={(event) => setSepticEditForm((current) => ({ ...current, lon: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div><div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Объём, м³<input required type="number" min="0.1" step="0.1" value={septicEditForm.tank_volume_m3} onChange={(event) => setSepticEditForm((current) => ({ ...current, tank_volume_m3: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label><label className="text-sm font-bold">Стоимость, ₽<input required type="number" min="1" step="1" value={septicEditForm.service_price} onChange={(event) => setSepticEditForm((current) => ({ ...current, service_price: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 font-normal" /></label></div></div>}<div className="mt-5 grid grid-cols-2 gap-3"><button type="button" onClick={() => setEditTarget(null)} className="rounded-xl bg-slate-100 py-3 font-bold text-slate-700">Отмена</button><button disabled={actionId === editTarget.data.id} className="flex items-center justify-center rounded-xl bg-sky-500 py-3 font-bold text-white disabled:opacity-50">{actionId === editTarget.data.id ? <Loader2 className="h-5 w-5 animate-spin" /> : "Сохранить"}</button></div></form></div> : null}
    </section>
  );
}
