...
Sprint 18 вводит независимый жизненный цикл размещений для точек забора и объявлений спецтехники: тестовый период, срок размещения, регулярное подтверждение актуальности, автоматическое скрытие и ручное продление оператором. Платёжный провайдер пока не подключён; единое поле срока подготовлено для будущего webhook оплаты.

Мобильный контур продолжает развиваться через PWA + Capacitor.
