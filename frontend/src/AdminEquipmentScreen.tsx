...
import { EquipmentListing, EquipmentTypeItem, formatEquipmentPrice } from "./EquipmentCatalogScreen";
...
export default function AdminEquipmentScreen({ applicationsOnly = false }: { applicationsOnly?: boolean }) {
  ...
  const openListing = (item?: EquipmentListing) => {
    ...
    } : { ...emptyListing, equipment_type_id: types[0]?.id || "" });
  ...
  const saveListing = async (event: React.FormEvent) => {
    ...
    const response = await fetch(`${baseURL}/admin/equipment${listingForm.id ? `/${listingForm.id}` : ""}`, {
      ...
      body: JSON.stringify({
        ...listingForm,
        id: undefined,
        price_amount: listingForm.price_unit === "negotiable" ? null : Number(listingForm.price_amount),
      }),
    });
  ...
  const changeApplicationStatus = async (item: Application) => {
    const nextStatus = item.status === "new" ? "in_progress" : "closed";
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === "listings" && <><div className="flex items-center justify-between"><div><h2 className="text-2xl font-black">Объявления спецтехники</h2><p className="text-sm text-slate-500">Публикации клиентского каталога</p></div><button onClick={() => openListing()} className="flex items-center gap-2 rounded-xl bg-sky-500 px-4 py-3 font-bold text-white"><Plus className="h-4 w-4" />Добавить</button></div><div className="grid gap-4 lg:grid-cols-2">{listings.map((item) => <div key={item.id} className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="relative">{item.primary_image_url ? <img src={resolveMediaUrl(item.primary_image_url) || "/placeholder.jpg"} className="h-48 w-full object-cover" alt={item.title} /> : <div className="flex h-48 items-center justify-center bg-slate-100"><ImageIcon className="h-10 w-10 text-slate-300" /></div>}<label className="absolute bottom-3 right-3 cursor-pointer rounded-xl bg-white p-2 shadow"><UploadCloud className="h-5 w-5 text-sky-500" /><input type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) void uploadPhoto(item.id, file); e.target.value = ""; }} /></label></div><div className="p-4"><p className="text-xs font-bold text-sky-600">{item.equipment_type_name}</p><div className="flex justify-between gap-2"><div><h3 className="text-lg font-black">{item.title}</h3><p className="font-bold">{formatEquipmentPrice(item)}</p></div><div className="flex"><button onClick={() => openListing(item)} className="p-2 text-slate-500"><Edit2 className="h-4 w-4" /></button><button onClick={() => void deleteListing(item.id)} className="p-2 text-rose-500"><Trash2 className="h-4 w-4" /></button></div></div><div className="mt-3 flex gap-2 overflow-x-auto">{item.media_files?.map((media) => <div key={media.id} className="relative shrink-0"><img src={resolveMediaUrl(media.public_url) || "/placeholder.jpg"} className="h-16 w-20 rounded-lg object-cover" /><button onClick={() => void mediaAction(media.id, "primary")} className={`absolute left-1 top-1 rounded-full p-1 ${media.is_primary ? "bg-amber-400 text-white" : "bg-white/90 text-slate-500"}`}><Star className="h-3 w-3" /></button><button onClick={() => void mediaAction(media.id, "delete")} className="absolute right-1 top-1 rounded-full bg-white/90 p-1 text-rose-500"><X className="h-3 w-3" /></button></div>)}</div></div></div>)}</div></>}

      {tab === "applications" && <div className="space-y-4"><div><h2 className="text-2xl font-black">Заявки на спецтехнику</h2><p className="text-sm text-slate-500">Обработка клиентских заявок</p></div>{applications.length === 0 ? <p className="rounded-2xl bg-white p-10 text-center text-slate-500">Новых заявок пока нет</p> : applications.map((item) => <div key={item.id} className="rounded-3xl bg-white p-5 shadow-sm"><div className="flex gap-4">{item.primary_image_url && <img src={resolveMediaUrl(item.primary_image_url) || "/placeholder.jpg"} className="h-24 w-28 rounded-xl object-cover" />}<div className="min-w-0 flex-1"><div className="flex items-start justify-between"><div><p className="text-xs font-bold text-sky-600">{item.status === "new" ? "НОВАЯ" : item.status === "in_progress" ? "В РАБОТЕ" : "ЗАКРЫТА"}</p><h3 className="text-lg font-black">{item.listing_title_snapshot}</h3></div></div><p className="mt-1 text-sm font-bold">{item.client_name} · {item.contact_phone}</p><p className="mt-2 text-sm text-slate-600">{item.object_address}</p><p className="mt-1 text-sm text-slate-500">{new Date(item.requested_date).toLocaleDateString("ru-RU")} в {item.requested_time.slice(0, 5)} · {item.duration_value} {item.duration_unit === "hours" ? "ч." : "смен"}</p></div></div>{item.comment && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm">{item.comment}</p>}{item.status !== "closed" && <button onClick={() => void changeApplicationStatus(item)} className={`mt-4 w-full rounded-xl p-3 font-bold ${item.status === "new" ? "bg-sky-500 text-white" : "bg-emerald-100 text-emerald-700"}`}>{item.status === "new" ? "Взять в работу" : "Закрыть заявку"}</button>}</div>)}</div>}

      {showListingForm && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/50 p-4"><form onSubmit={saveListing} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-6"><div className="mb-5 flex justify-between"><h3 className="text-xl font-black">{listingForm.id ? "Редактировать" : "Новое объявление"}</h3><button type="button" onClick={() => setShowListingForm(false)}><X /></button></div><div className="space-y-4"><label className="block text-sm font-bold">Тип<select required value={listingForm.equipment_type_id} onChange={(e) => setListingForm({ ...listingForm, equipment_type_id: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal">{types.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="block text-sm font-bold">Название<input required value={listingForm.title} onChange={(e) => setListingForm({ ...listingForm, title: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal" /></label><label className="block text-sm font-bold">Описание<textarea required rows={5} value={listingForm.description} onChange={(e) => setListingForm({ ...listingForm, description: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal" /></label><div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Тариф<select value={listingForm.price_unit} onChange={(e) => setListingForm({ ...listingForm, price_unit: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal"><option value="hour">За час</option><option value="shift">За смену</option><option value="day">За день</option><option value="negotiable">Договорная</option></select></label>{listingForm.price_unit !== "negotiable" && <label className="text-sm font-bold">Цена<input required type="number" min="1" value={listingForm.price_amount} onChange={(e) => setListingForm({ ...listingForm, price_amount: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal" /></label>}</div><div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Город<input value={listingForm.city} onChange={(e) => setListingForm({ ...listingForm, city: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal" /></label><label className="text-sm font-bold">Район<input value={listingForm.district} onChange={(e) => setListingForm({ ...listingForm, district: e.target.value })} className="mt-1 w-full rounded-xl bg-slate-100 p-3 font-normal" /></label></div><label className="flex items-center gap-3 rounded-xl bg-slate-50 p-3"><input type="checkbox" checked={listingForm.is_active} onChange={(e) => setListingForm({ ...listingForm, is_active: e.target.checked })} />Активное объявление</label></div><button className="mt-5 w-full rounded-xl bg-sky-500 p-4 font-bold text-white">Сохранить</button></form></div>}
    </div>
  );
}
