...
import { Building2, Edit2, Loader2, MapPin, Plus, RefreshCw, Search, X } from "lucide-react";
...
export default function AdminCitiesScreen({ onClose, modal = false }: { onClose?: () => void; modal?: boolean }) {
  ...
  useEffect(() => {
    if (!draft || draft.name.trim().length < 3) { setSuggestions([]); return undefined; }
    let cancelled = false;
  ...
  const closeDialog = () => { setDraft(null); setSuggestions([]); };
  const openEdit = (city: City) => setDraft({ id: city.id, name: city.name, region: city.region, center_lat: String(city.center_lat), center_lon: String(city.center_lon), map_zoom: String(city.map_zoom) });
