...
export default function NotificationCenter({ token }: { token: string | null }) {
  ...
  return <>
    ...
    {isOpen ? <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/30" role="dialog" aria-modal="true" aria-label="Центр уведомлений">
      <div className="flex h-full w-full max-w-md flex-col bg-slate-50 shadow-2xl">
        <header className="flex items-center justify-between border-b bg-white p-4"><div><h2 className="text-lg font-black">Уведомления</h2><p className="text-sm text-slate-500">Новые заявки и события</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => void markAllRead()} className="rounded-lg p-2 text-sky-600" aria-label="Прочитать все"><CheckCheck /></button><button type="button" onClick={() => setIsOpen(false)} className="rounded-lg p-2 text-slate-600" aria-label="Закрыть"><X /></button></div></header>
        <div className="flex-1 overflow-y-auto p-4">{loading ? <Loader2 className="mx-auto mt-8 animate-spin text-sky-500" /> : null}{!loading && items.length === 0 ? <p className="rounded-2xl bg-white p-4 text-sm text-slate-500">У вас пока нет уведомлений.</p> : null}<div className="space-y-3">{items.map((item) => <article key={item.id} className={`rounded-2xl p-4 shadow-sm ${item.is_read ? "bg-white" : "bg-sky-50 ring-1 ring-sky-200"}`}><h3 className="font-bold text-slate-900">{item.title}</h3><p className="mt-1 text-sm text-slate-600">{item.body}</p>{item.created_at ? <time className="mt-2 block text-xs text-slate-400">{new Date(item.created_at).toLocaleString("ru-RU")}</time> : null}</article>)}</div></div>
      </div>
    </div> : null}
  </>;
}
