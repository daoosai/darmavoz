...
test('калькулятор показывает объём, тоннаж и загрузки без запроса цены', async ({ page }) => {
  ...
  await expect(dialog.getByLabel('Кубатура машины, м³', { exact: true })).toHaveValue('');
  await dialog.getByLabel('Кубатура машины, м³', { exact: true }).fill('20');
  await dialog.getByLabel('Рассчитать ориентировочный тоннаж').check();
  await expect(dialog.getByText('Нужно 50 м³ материала')).toBeVisible();
  await expect(dialog.getByText('Ориентировочный тоннаж: 75 т')).toBeVisible();
  await expect(dialog.getByText('Машин/рейсов: 3. Загрузки: 20 + 20 + 10 м³.')).toBeVisible();
